...
void GdaFrame::UpdateToolbarAndMenus()
{
	...
	GeneralWxUtils::EnableMenuItem(mb, XRCID("ID_GWRMODEL_TEST"), proj_open);
...
void GdaFrame::OnGwrmodelTest(wxCommandEvent& event)
{
	//wxMessageDialog("test success");
	Project* p=GetProject();

	if (p)
	{
		GWRDlg* dlg=new GWRDlg(project_p,this);
		dlg->Show(true);

		/*wxDialog dlg;
		wxXmlResource::Get()->LoadDialog(&dlg,this,"IDG_GWRMODEL");
		dlg.ShowModal();*/
	}
}
...
BEGIN_EVENT_TABLE(GdaFrame, wxFrame)
	...
	EVT_TOOL(XRCID("ID_GWRMODEL_TEST"),GdaFrame::OnGwrmodelTest)
